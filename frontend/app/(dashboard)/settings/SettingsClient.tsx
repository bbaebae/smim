...
export default function SettingsClient({ email, initialEmailNotify, initialPushNotify }: Props) {
  const [emailNotify, setEmailNotify] = useState(initialEmailNotify)
  const [pushNotify, setPushNotify] = useState(initialPushNotify)
  ...
  async function handlePushToggle() {
    const next = !pushNotify
    setSaving(true)
    try {
      if (next) {
        const permission = await Notification.requestPermission()
        if (permission !== 'granted') {
          setMsg('브라우저 알림 권한이 필요합니다.')
          setTimeout(() => setMsg(''), 3000)
          setSaving(false)
          return
        }
        const sub = await subscribePush()
        if (!sub) throw new Error('구독 실패')
  ...
  return (
    <div className="min-h-screen bg-[#f5f3f3] p-6 md:p-10">
      <div className="max-w-lg mx-auto">
        ...
        {/* 알림 설정 */}
        <section className="bg-white rounded-2xl border border-[#e4e2e2] overflow-hidden mb-4" style={{ boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
          <div className="px-6 py-4 border-b border-[#f0eeee]">
            <h2 className="text-[15px] font-semibold text-[#1b1c1c]">복습 알림</h2>
            <p className="text-[13px] text-[#767683] mt-0.5">이메일 또는 앱 푸시 알림으로 복습 일정을 안내해 드립니다.</p>
          </div>

          {/* 이메일 알림 */}
          <div className="px-6 py-4 flex items-center justify-between border-b border-[#f0eeee]">
            <div>
          ...
          <div className="px-6 py-4 flex items-center justify-between">
            <div>
              <p className="text-[14px] font-medium text-[#1b1c1c]">앱 푸시 알림</p>
              <p className="text-[12px] text-[#a0a0b0] mt-0.5">브라우저 알림으로 복습 시간을 놓치지 마세요</p>
            </div>
            <button
              ...
              className={[
                'relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none',
                pushNotify ? 'bg-[#132175]' : 'bg-[#d4d3db]',
              ].join(' ')}
            >
              <span className={[
                'inline-block h-5 w-5 transform rounded-full bg-white shadow-sm transition-transform',
                pushNotify ? 'translate-x-5.5' : 'translate-x-0.5',
              ].join(' ')} />
            </button>
          </div>
        </section>
